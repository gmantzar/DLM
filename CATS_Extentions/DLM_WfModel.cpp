...
DLM_Histo<complex<double>> ***Init_pp_Norfolk(const char *CatsFolder, CATS *Kitty, const int &TYPE) {
  if (!Kitty)
    return NULL;
  return Init_pp_Norfolk(CatsFolder, *Kitty, TYPE);
}
...
DLM_Histo<complex<double>> ***Init_pp_Norfolk(const char *CatsFolder, CATS &Kitty, const int &TYPE) {
  ...
  // all files should have the same momentum and radius bins so we just get them
  // from the 1S0 file, which is the first one in the array of file names
  std::string FileName = path + std::string("wavefunctions_NV2-IIA_EM2.root");
  TFile *Input = new TFile(FileName.c_str(), "READ");
  ...
  // grab one 2d histogram to get binning for momentum and radius
  TH2F *Hist2D = static_cast<TH2F *>(
      Input->Get("1S0/wf2D"));

  // for a fixed momentum we get the binning of the radius
  for (int i = 0; i < Hist2D->GetNbinsY(); i++) {
    RadiusBinEdges.push_back(Hist2D->GetYaxis()->GetBinLowEdge(i + 1));
  }
  RadiusBinEdges.push_back(Hist2D->GetYaxis()->GetBinUpEdge(Hist2D->GetNbinsY()));
  // for a fixed radius we get the binning of the momentum
  for (int i = 0; i < Hist2D->GetNbinsX(); i++) {
    MomentumBinEdges.push_back(Hist2D->GetXaxis()->GetBinLowEdge(i + 1));
  }
  MomentumBinEdges.push_back(Hist2D->GetXaxis()->GetBinUpEdge(Hist2D->GetNbinsX()));
